...
import br.com.coopersystem.util.Resposta;
...
public class CotacaoResourse {
	...
	@APIResponse(responseCode = "200", description = "Tudo funcionou como esperado.", content = @Content(schema = @Schema(implementation = Resposta.class)))
	public Response cotacaoDolarDia(@QueryParam("dataCotacao") String dataCotacao) {
		// fiz apenas para final de semana, não fiz para feriados específicos
		try {
			...
			if (!Util.isDayUtil(data)) {
				...
				String diaAnterior = Util.diaUltilAnterior(data);
				Cotacao dataInformada = cotacaoService.cotacaoDolarDia(dataCotacao).readEntity(Cotacao.class);
				Cotacao dataDiaAnterior = cotacaoService.cotacaoDolarDia("'" + diaAnterior + "'")
						.readEntity(Cotacao.class);
				Resposta resposta = new Resposta(dataInformada, dataDiaAnterior);
